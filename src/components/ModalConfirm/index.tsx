import { DeleteModalType } from "../../interfaces/common";
...
const ModalConfirm = ({ isOpen, onClose, onConfirm, type }: ModalConfirmProps) => {
    if (!isOpen) {
        return null;
    }
    ...
    return (
        <S.ModalOverlay onClick={onClose}>
            <S.Modal onClick={(e) => e.stopPropagation()}>
                <S.TopBar>
                    <S.CloseButton onClick={onClose} type="button">&#x2715;</S.CloseButton>
                </S.TopBar>
                <S.Header>
                    <p>{type === 'delete' ? 'Excluir ponto selecionado?' : 'Excluir todos os pontos?'}</p>
                </S.Header>
